...
const C = {
  ...
  gradientPrimary: "linear-gradient(135deg, #1E40AF 0%, #2563EB 50%, #06B6D4 100%)",
...
const RESULTADOS = [
  ...
  { id:"reagendada",     label:"Reagendada",        emoji:"📅", color:C.accent   },
];
...
const NIVEL_INTERES = [
  ...
  { val:2, label:"Escuchó apenas",  desc:"Atendió por compromiso, sin engagement",      color:C.orange  },
  { val:3, label:"Interés normal",  desc:"Escuchó y hizo preguntas básicas",            color:C.warn    },
  { val:4, label:"Interés alto",    desc:"Pidió más información o muestras",            color:"#22C55E" },
...
// ── AUTH SERVICE ─────────────────────────────────────────────────
// Manejo centralizado de tokens con refresh automático
const AuthService = {
  ...
  // Fetch con interceptor de 401 y refresh automático
      ...
      <div style={{
        ...
        <div style={{textAlign:"center"}}>
          <div style={{
            ...
            background:C.gradientPrimary, WebkitBackgroundClip:"text",
            WebkitTextFillColor:"transparent",marginBottom:8,
            fontFamily:"'Space Grotesk', sans-serif",
          }}>SIDM</div>
          <div style={{fontSize:14,color:C.muted}}>Cargando...</div>
          <div style={{
            width:32,height:3,borderRadius:2,background:C.gradientAccent,
            margin:"16px auto 0",
...
function PantallaLogin({ onLogin }) {
  ...
  return (
    <div style={{
      ...
      padding:20, fontFamily:fontStack,
    }}>
      {/* Decorative background */}
      <div style={{
        position:"fixed", top:0, left:0, right:0, height:"45vh",
        background:C.gradientPrimary, borderRadius:"0 0 40px 40px",
        zIndex:0,
      }} />
      <div style={{
        position:"fixed", top:0, left:0, right:0, height:"45vh",
      ...
      <div style={{
        ...
        boxShadow:"0 20px 60px rgba(15,23,42,0.15), 0 4px 20px rgba(15,23,42,0.08)",
      }}>
        <div style={{textAlign:"center",marginBottom:32}}>
          ...
          <p style={{fontSize:14,color:C.muted,margin:0,lineHeight:1.5}}>
            Sistema de Inteligencia de<br/>Disponibilidad Médica
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <button
            ...
            style={{
              ...
              color:C.white,fontFamily:fontStack,letterSpacing:"0.01em",
              border:"none",borderRadius:12,cursor:cargando?"not-allowed":"pointer",
              boxShadow:cargando ? "none" : "0 4px 14px rgba(30,64,175,0.35)",
              transition:"all 0.2s",
            }}
          >
        ...
        <p style={{
          fontSize:11,color:C.muted,marginTop:24,textAlign:"center",letterSpacing:"0.02em",
        }}>
          Piloto Inbiotech · v0.2.1
        </p>
      </div>
    </div>
  );
...
function AppPrincipal({ user, onLogout, onSessionExpired }) {
  ...
  // Fetch seguro con manejo de sesión expirada
  const apiFetch = useCallback(async (path, options = {}) => {
    try {
      return await AuthService.secureFetch(`${API_URL}${path}`, options);
    } catch (err) {
  ...
  const cargarHistorial = useCallback(async () => {
    try {
      const [resH, resS] = await Promise.all([
        apiFetch("/visitas/historial?limite=100"),
        apiFetch("/visitas/stats"),
      ]);
        ...
        <div ref={menuRef} style={{position:"relative"}}>
          <button
            ...
            style={{
              ...
              background:C.gradientPrimary, border:"none",
          ...
          {showMenu && (
            <div style={{
              ...
              background:C.white, border:`1px solid ${C.border}`,
              borderRadius:12, boxShadow:C.shadowLg,
              minWidth:200, overflow:"hidden", zIndex:30,
            }}>
              <div style={{
                padding:"14px 16px", borderBottom:`1px solid ${C.border}`,
              }}>
                <div style={{fontSize:14,fontWeight:600,color:C.textMain}}>
                  {user?.nombre}
                </div>
                ...
                <div style={{
                  fontSize:11, color:C.action, fontWeight:500,
                  marginTop:4, textTransform:"capitalize",
                }}>
                  {user?.laboratorio} · {user?.rol || "visitador"}
                </div>
              </div>
              <button
                ...
                style={{
                  ...
                  cursor:"pointer", color:C.textSub, fontFamily:fontStack,
                  borderBottom:`1px solid ${C.border}`,
                }}
                onMouseEnter={e => e.target.style.background=C.section}
              ...
              <button
                ...
                style={{
                  width:"100%", padding:"11px 16px", fontSize:13,
                  background:"none", border:"none", textAlign:"left",
                  cursor:"pointer", color:C.danger, fontFamily:fontStack,
                }}
      ...
      <div style={{
        ...
        {tabs.map(t => (
          <button key={t.id} onClick={() => { setTab(t.id); if(t.id==="nueva") resetFormulario(); }}
            style={{
              flex:1, padding:"12px 0", fontSize:13, fontWeight:tab===t.id?600:400,
              fontFamily:fontStack,
              background:tab===t.id?C.bg:C.white,
              color:tab===t.id?C.action:C.muted,
              border:"none",
              borderBottom:tab===t.id?`2.5px solid ${C.action}`:"2.5px solid transparent",
              ...
              {[1,2,3,4].map(p => (
                <div key={p} style={{display:"flex",alignItems:"center",gap:4,flex:p<4?1:"none"}}>
                  <div style={{
                    ...
                    background: p < paso ? C.action : p === paso ? `${C.action}12` : C.white,
                    color: p < paso ? C.white : p === paso ? C.action : C.muted,
                    border: p === paso ? `2px solid ${C.action}` : p < paso ? "none" : `1.5px solid ${C.border}`,
                    transition:"all 0.3s",
                    boxShadow: p < paso ? `0 2px 8px ${C.action}40` : "none",
                  }}>
                    {p < paso ? "✓" : p}
                  </div>
                  {p < 4 && (
                    <div style={{
                      ...
                      background: p < paso ? C.action : C.border,
                      transition:"all 0.3s",
                    }}/>
                  )}
                </div>
...
function CambiarPassword({ apiFetch, onBack, onLogout }) {
  ...
  return (
    <div style={{
      minHeight:"100vh", background:C.bg, fontFamily:fontStack,
      ...
      <div style={{width:"100%",maxWidth:440,marginTop:40}}>
        ...
        <div style={{
          ...
          <h2 style={{fontSize:20,fontWeight:600,margin:"0 0 4px",color:C.textMain}}>
            Cambiar contraseña
          </h2>
          <p style={{fontSize:13,color:C.muted,margin:"0 0 24px"}}>
            Se cerrará tu sesión al cambiarla
          </p>
          ...
          {success ? (
            <div style={{
              ...
              <div style={{fontSize:15,fontWeight:600,color:C.success}}>
            ...
            <>
              ...
              {newPwd.length > 0 && (
                <div style={{
                  background:C.section, borderRadius:10, padding:"12px 14px",
                  marginBottom:16,
                }}>
                  {validations.map((v, i) => (
                    <div key={i} style={{
                      fontSize:12, color: v.ok ? C.success : C.muted,
                      display:"flex", alignItems:"center", gap:6,
                      marginBottom: i < validations.length-1 ? 4 : 0,
                    }}>
              ...
              <BtnPrimario disabled={!allValid || loading} onClick={handleSubmit}>
    ...
    <div>
      ...
      <div style={{maxHeight:380,overflowY:"auto",marginBottom:14}}>
        ...
        {filtrados.map(m => {
          ...
          return (
            <div key={m.id} onClick={() => onSeleccionar(m)}
              style={{
                background: isSelected ? `${C.action}08` : C.white,
                border: `1.5px solid ${isSelected ? C.action : C.border}`,
                borderRadius:12, padding:"12px 14px", marginBottom:8,
                cursor:"pointer", transition:"all 0.15s",
                boxShadow: isSelected ? `0 0 0 3px ${C.action}12` : C.shadow,
              }}
            >
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                ...
                {sppPct !== null && (
                  <div style={{
                    background:`${sppColor}12`, border:`1px solid ${sppColor}30`,
                    borderRadius:8, padding:"4px 10px", fontSize:12,
                    fontWeight:600, color:sppColor, whiteSpace:"nowrap",
                  }}>
                    {sppPct}%
                  </div>
                )}
              ...
              {m.total_visitas > 0 && (
                <div style={{fontSize:11,color:C.muted,marginTop:6}}>
                  {m.total_visitas} visitas · confianza {m.confianza}
                </div>
              )}
            </div>
          );
...
function PasoLugar({ medico, consultorio, setConsultorio, horaLlegada, setHoraLlegada, canalContacto, setCanalContacto, onAtras, onSiguiente }) {
  ...
  return (
    <div>
      ...
      <p style={{fontSize:13,color:C.textSub,margin:"-8px 0 20px"}}>
        ...
        {medico.consultorios?.length > 0 ? (
          <div style={{display:"flex",flexWrap:"wrap",gap:8,marginBottom:8}}>
            {medico.consultorios.map(c => (
              <button key={c} onClick={() => setConsultorio(c)}
                style={{
                  padding:"8px 14px",fontSize:13,fontFamily:fontStack,
                  background:consultorio===c?`${C.action}10`:C.white,
                  border:`1.5px solid ${consultorio===c?C.action:C.border}`,
                  borderRadius:8,color:consultorio===c?C.action:C.textSub,
      ...
      <div style={{marginBottom:20}}>
        ...
        <div style={{display:"flex",gap:8}}>
          {CANALES.map(c => (
            <button key={c.id} onClick={() => setCanalContacto(c.id)}
              style={{
                flex:1,padding:"10px 8px",fontSize:13,fontFamily:fontStack,
                background:canalContacto===c.id?`${C.action}10`:C.white,
                border:`1.5px solid ${canalContacto===c.id?C.action:C.border}`,
                borderRadius:10,color:canalContacto===c.id?C.action:C.textSub,
                cursor:"pointer",transition:"all .15s",
...
function PasoResultado({
  ...
  nivelInteres, setNivelInteres, pacientesEnSala, setPacientesEnSala,
  horaInicioAtencion, setHoraInicioAtencion, onAtras, onSiguiente,
}) {
    ...
    <div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:8}}>
          {RESULTADOS.map(r => (
            <button key={r.id} onClick={() => setResultado(r.id)}
              style={{
                padding:"14px 8px",fontSize:13,fontFamily:fontStack,
                background:resultado===r.id?`${r.color}10`:C.white,
                border:`1.5px solid ${resultado===r.id?r.color:C.border}`,
                borderRadius:10,color:resultado===r.id?r.color:C.textSub,
                cursor:"pointer",display:"flex",flexDirection:"column",
                alignItems:"center",gap:5,transition:"all .15s",
                boxShadow:resultado===r.id?`0 0 0 3px ${r.color}10`:"none",
              }}>
      ...
      {esConMedico && (
        <div style={sectionStyle}>
          ...
          <div style={{display:"flex",gap:6,marginBottom:6}}>
            {NIVEL_INTERES.map(n => (
              <button key={n.val} onClick={() => setNivelInteres(n.val)}
                style={{
                  flex:1,background:nivelInteres===n.val?`${n.color}10`:C.white,
                  border:`1.5px solid ${nivelInteres===n.val?n.color:C.border}`,
                  borderRadius:10,padding:"10px 0",cursor:"pointer",fontFamily:fontStack,
          ...
          {nivelInteres && (
            <div style={{
              background:`${NIVEL_INTERES[nivelInteres-1].color}08`,
              border:`1px solid ${NIVEL_INTERES[nivelInteres-1].color}25`,
              borderRadius:10,padding:"8px 12px",
            }}>
              <span style={{fontSize:13,fontWeight:600,color:NIVEL_INTERES[nivelInteres-1].color}}>
      ...
      <div style={sectionStyle}>
        ...
        <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
          {ESPERA_OPCIONES.map(e => (
            <button key={e.val} onClick={() => setEspera(espera===e.val?null:e.val)}
              style={{
                padding:"8px 14px",fontSize:12,fontFamily:fontStack,
                background:espera===e.val?`${C.accent}10`:C.white,
                border:`1.5px solid ${espera===e.val?C.accent:C.border}`,
                borderRadius:8,color:espera===e.val?C.accent:C.textSub,
                cursor:"pointer",transition:"all .15s",
              }}>
              {e.label}
            </button>
      ...
      <div style={sectionStyle}>
        ...
        <div style={{display:"flex",gap:6}}>
          {[{val:"vacio",label:"Vacío"},{val:"1-3",label:"1-3"},{val:"4-6",label:"4-6"},{val:"lleno",label:"Lleno"}].map(op => (
            <button key={op.val} onClick={() => setPacientesEnSala(pacientesEnSala===op.val?null:op.val)}
              style={{
                flex:1,padding:"9px 0",fontSize:13,fontFamily:fontStack,
                background:pacientesEnSala===op.val?`${C.accent}10`:C.white,
                border:`1.5px solid ${pacientesEnSala===op.val?C.accent:C.border}`,
                borderRadius:8,color:pacientesEnSala===op.val?C.accent:C.textSub,
                cursor:"pointer",transition:"all .15s",
              }}>
              {op.label}
            </button>
      ...
      <div style={sectionStyle}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:6}}>
          {NOVEDADES.map(n => (
            <button key={n.id} onClick={() => setNovedadCat(novedadCat===n.id?null:n.id)}
              style={{
                padding:"8px 10px",fontSize:12,textAlign:"left",fontFamily:fontStack,
                background:novedadCat===n.id?`${C.warn}10`:C.white,
                border:`1.5px solid ${novedadCat===n.id?C.warn:C.border}`,
                borderRadius:8,color:novedadCat===n.id?C.warn:C.textSub,
...
function PasoConfirmacion({
  ...
  pacientesEnSala, horaInicioAtencion, enviando, onAtras, onEnviar,
}) {
  ...
  return (
    <div>
      ...
      <div style={{
        ...
        <div style={{marginBottom:8}}>
          <div style={{
            ...
            borderRadius:8,fontSize:13,fontWeight:500,color:res.color,
          }}>
            {res.emoji} {res.label}
          </div>
        </div>
        ...
        {nivelInteres && (() => {
          ...
          return (
            <div style={{
              ...
              background:`${ni.color}08`,border:`1px solid ${ni.color}20`,
              ...
              <div style={{display:"flex",alignItems:"center",gap:8}}>
                <div style={{display:"flex",gap:3}}>
                  {[1,2,3,4,5].map(n => (
                    <div key={n} style={{width:7,height:7,borderRadius:"50%",background:n<=nivelInteres?ni.color:C.border}}/>
                  ))}
                </div>
        ...
        {nota && (
          <div style={{marginTop:10}}>
            ...
            <div style={{
              ...
              background:C.section,borderRadius:8,padding:"8px 12px",
      ...
      <button disabled={enviando} onClick={onEnviar}
        style={{
          width:"100%",padding:14,fontSize:16,fontWeight:700,
          fontFamily:fontStack,letterSpacing:"0.01em",
          background:enviando?C.muted:C.gradientPrimary,
          color:C.white,border:"none",borderRadius:12,
          cursor:enviando?"not-allowed":"pointer",
          boxShadow:enviando?"none":"0 4px 14px rgba(30,64,175,0.35)",
          transition:"all .2s",
        }}>
  ...
  return (
    <div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10,marginBottom:18}}>
        <StatCard label="Visitas" value={stats.total_visitas} color={C.action} />
          ...
          <div key={h.id} style={{
            ...
            {h.nivel_interes && (
              <div style={{marginTop:8,display:"flex",alignItems:"center",gap:4}}>
                {[1,2,3,4,5].map(n => (
                  <div key={n} style={{width:6,height:6,borderRadius:"50%",background:n<=h.nivel_interes?NIVEL_INTERES[h.nivel_interes-1].color:C.border}}/>
                ))}
  ...
  return (
    <div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:10,marginBottom:20}}>
        <StatCard label="Total visitas" value={stats.total_visitas} color={C.action} />
      ...
      <ChartCard title="Resultados por tipo">
        <ResponsiveContainer width="100%" height={220}>
          <BarChart data={datosResultados} margin={{top:5,right:10,left:-15,bottom:5}}>
            ...
            <Tooltip content={<CustomTooltip/>} cursor={{fill:`${C.action}08`}} />
        ...
        <ChartCard title="Visitas por día">
          <ResponsiveContainer width="100%" height={200}>
            <LineChart data={datosPorDia} margin={{top:5,right:10,left:-15,bottom:5}}>
              ...
              <YAxis tick={{fill:C.muted,fontSize:11}} axisLine={false} tickLine={false} allowDecimals={false} />
              <Tooltip content={<CustomTooltip/>} />
              <Line type="monotone" dataKey="total" name="Total" stroke={C.action} strokeWidth={2} dot={{fill:C.action,r:3}} />
              <Line type="monotone" dataKey="exitosas" name="Exitosas" stroke={C.success} strokeWidth={2} dot={{fill:C.success,r:3}} />
            </LineChart>
          </ResponsiveContainer>
          <ChartLegend items={[{color:C.action,label:"Total"},{color:C.success,label:"Exitosas"}]} />
        </ChartCard>
      )}
        ...
        <ChartCard title="Visitas por médico">
          <ResponsiveContainer width="100%" height={Math.max(150, datosPorMedico.length * 45)}>
            <BarChart data={datosPorMedico} layout="vertical" margin={{top:5,right:10,left:10,bottom:5}}>
              ...
              <Tooltip content={<CustomTooltip/>} cursor={{fill:`${C.action}08`}} />
              <Bar dataKey="total" name="Total" fill={C.action} radius={[0,6,6,0]} barSize={14} />
              <Bar dataKey="efectivas" name="Efectivas" fill={C.success} radius={[0,6,6,0]} barSize={14} />
            </BarChart>
          </ResponsiveContainer>
          <ChartLegend items={[{color:C.action,label:"Total"},{color:C.success,label:"Efectivas"}]} />
        </ChartCard>
      )}
    </div>
  );
...
function StatCard({ label, value, color = C.action }) {
  return (
    <div style={{
      background:C.white, border:`1px solid ${C.border}`,
...
function ChartCard({ title, children }) {
  return (
    <div style={{
      background:C.white, border:`1px solid ${C.border}`,
      borderRadius:14, padding:"16px 12px", marginBottom:14,
      boxShadow:C.shadow,
    }}>
...
function BtnAtras({ onClick }) {
  return (
    <button onClick={onClick} style={{
      background:"none",border:"none",color:C.action,
  ...
  return (
    <button disabled={disabled} onClick={onClick}
      style={{
        ...
        boxShadow:disabled?"none":"0 4px 14px rgba(30,64,175,0.3)",
